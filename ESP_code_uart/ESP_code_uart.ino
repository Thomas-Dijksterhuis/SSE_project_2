...
void readExact(uint8_t* buf, size_t len) {
  size_t got = 0;
  while (got < len) {
    int avail = Serial2.available();
    if (avail <= 0) {
      vTaskDelay(1);  // yield for 1ms, don't starve other tasks
      continue;
    }
    got += Serial2.readBytes((char*)buf + got, min((size_t)avail, len - got));
  }
}

// Block until two consecutive 0xAA sync bytes are seen.
void syncToFrame() {
  uint8_t prev = 0;
  while (true) {
    int b = Serial2.read();
    ...
    if ((uint8_t)b == SYNC_BYTE && prev == SYNC_BYTE) return;
    prev = (uint8_t)b;
  }
}
...
void audioProcessTask(void *pv) {
  static FRAME_FORMAT frame[FRAME_SAMPLES];  // stereo: [piezo, mic] interleaved
  ...
  while (true) {
    ...
    readExact(count_bytes, 2);
    uint16_t n = (uint16_t)count_bytes[0] | ((uint16_t)count_bytes[1] << 8);

    if (n == 0 || n > SAMPLES_PER_CHANNEL) {
      continue;
    }

    // 3. Read payload directly into int16_t buffer
    size_t payload_bytes = n * CHANNELS * sizeof(FRAME_FORMAT);
    readExact((uint8_t*)frame, payload_bytes);

    // for (int i = 0; i < FRAME_SAMPLES; i+=2){
    //   Serial.print("ADC:");
    //   Serial.print(frame[i]);
    //   Serial.print(", MIC:");
    //   Serial.println(frame[i+1]);
    // }

    // 4. Same logic as before
    if (!sdReady || !systemActive) continue;
...
void setup() {
  ...
  Serial2.setRxBufferSize(2048);  // must be before begin(); default 256 is too small
